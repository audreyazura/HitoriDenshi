        ...
        for (Absorber currentAbsorber: m_absorbers)
        {
            ...
            try
            {
                ...
                for (BigDecimal initialPosition: initialPositionWithNotch)
                {
                    ...
                    currentTracker.saveToFile(m_outputFolder + "/" + currentAbsorber.getFileName(), initialPosition.divide(PhysicsVariables.UnitsPrefix.NANO.getMultiplier(), MathContext.DECIMAL128), m_abscissaUnit);
                    m_manager.sendMessage("SimulationWorker-"+String.valueOf(m_id)+": Calculation ended for " + currentAbsorber.getFileName() + " and x_init = "+String.valueOf((initialPosition.divide(PhysicsVariables.UnitsPrefix.NANO.getMultiplier(), MathContext.DECIMAL128)).intValue())+"nm.");
                }
            }
