...
public class Absorber
{
    ...
    {
        String absorberFile = p_sample.getConfigFile().getName();
        String[] splittedAbsorberFile = absorberFile.split("\\.");
        String fileExtension = splittedAbsorberFile[splittedAbsorberFile.length - 1];
        
        m_absorberFileName = absorberFile.split("\\." + fileExtension)[0];
        ...
        gradingValues.put(m_frontPosition, CalculationConditions.formatBigDecimal(grading.get("front").multiply(PhysicsVariables.EV)));
        gradingValues.put(m_notchPosition, CalculationConditions.formatBigDecimal(grading.get("notchgap").multiply(PhysicsVariables.EV)));
        gradingValues.put(m_backPosition, CalculationConditions.formatBigDecimal(grading.get("back").multiply(PhysicsVariables.EV)));
...
//        System.out.println("");
        ...
        return CalculationConditions.formatBigDecimal((p_gradingProfile.getValueAtPosition(m_notchPosition).subtract(p_gradingProfile.getValueAtPosition(interfacePosition))).divide(PhysicsVariables.Q.multiply(m_notchPosition.subtract(interfacePosition)), MathContext.DECIMAL128));
    }
