...
{    
    ...
    static final BigDecimal DT = CalculationConditions.formatBigDecimal(PhysicsVariables.UnitsPrefix.FEMTO.getMultiplier());
    ...
    private final PhysicsVariables.UnitsPrefix m_abscissaUnit;
    ...
    public CalculationConditions (List<Sample> sampleList, boolean p_isElectron, boolean p_isZeroAtFront, PhysicsVariables.UnitsPrefix p_prefix, int p_numberSimulatedParticules, BigDecimal p_effectiveMass, BigDecimal p_lifeTime, BigDecimal p_bufferWindowSize, BigDecimal p_absorberSize, String p_biasVoltages, String p_startingPositions)
    {
        ...
        BigDecimal particleEffectiveMass = formatBigDecimal(p_effectiveMass.multiply(PhysicsVariables.ME));
        ...
        if(p_isElectron)
        {
            m_particleParameters.put("charge", formatBigDecimal(PhysicsVariables.Q.negate()));
        }
        else
        {
            m_particleParameters.put("charge", formatBigDecimal(PhysicsVariables.Q));
        }
        ...
        */
        BigDecimal vth = formatBigDecimal((PhysicsVariables.KB.multiply(T).divide(particleEffectiveMass, MathContext.DECIMAL128)).sqrt(MathContext.DECIMAL128));
    ...
    public synchronized PhysicsVariables.UnitsPrefix getAbscissaScale()
    {
